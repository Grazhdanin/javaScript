...
const game = {
    ...
    start() {
        ...
        let ans = this.getNextQuestionId(firstQuestion);

        while (+nextQuestionId !== -1) {
            this.log(nextQuestionId + ' ' + ans);
            nextQuestionId = ans[nextQuestionId - 1];

            const nextQuestion = this.textNodes.find((q) => q.id === +nextQuestionId);
            ans = this.getNextQuestionId(nextQuestion);
    ...
    getNextQuestionId(question) {
        ...
        for (let i = 0; i < question.options.length; i++) {
            id += question.options[i].nextText;
        }
